 ...
 if(!setrole[message.guild.id]) setrole[message.guild.id] = {
   ...
   let ticketnumber = 0000;
    ...
    if (message.channel.name.startsWith("ticket-" + ticketnumber)) return message.channel.send(already);
     ...
     .setColor("22BF41")
     .setFooter(`Codes Server || Julian` , client.user.avatarURL)
     .setTimestamp();
...
if(message.content.toLowerCase().startsWith(prefix + `close`)) {   
    let team = message.member.roles.find("name", "Support Team");
     const d11x1xx = new Discord.RichEmbed()
     .setDescription(":x: You do not have permission for that command! If you believe this is a mistake please add the role called \`\`Support Team\`\` to yourself.")  
     .setColor("22BF41");
